...
local slotsEnum = require("slots.enum")
...
function logic:getScatterFrees()
    ---@type slots_cfg
    local cfg = self:getGameConf()
...
---@param x number @转轴
---@return slots_wight_info
function logic:getAxleIconWeithts(x)
...
---@param icon  slots_icon  @图标
---@return slots_wight
function logic:getAxleWeithtBy(x,icon)
...
---@param x number @x
---@param y number @y
function logic:createIcon(x,y,icons)
    local layt = self.getLayoutInfo()
    local wgts = self:getAxleIconWeithts(x)
...
function logic:rotateNormal(result)
...
---免费摇奖
---@return slots_result_normal
function logic:rotateFree(result)
    ---@type slots_result_normal
    result = result or {nil}
    local game_type     = slotsEnum.fpin()
...
---重转摇将
---@return slots_result_normal
function logic:rotateRoller(result)
    --计算成本
    local player = self:getCurPlayer()
...
---@return count
function logic:getFreePush(result)
    ---@type slots_cfg
    local cfg = self:getGameConf()
...
function logic:lineCheck(path,icons,path_index,left_right)
...
---@param result slots_result_normal
---@return score
function logic:getGainCoin(result)
    local score = 0
...
---@param result   slots_result_normal 
---@return count
function logic:getFreeLeft(result)
    local player = self:getCurPlayer()
...
function logic:getHeavyCost(result)
    ---@class jjbx_game_cfg
    local cfg = self:getGameConf()
...
function logic:getAxlesIons(icnlis)
    local axles = {
        [1] = {nil},
        [2] = {nil},
        [3] = {nil},
