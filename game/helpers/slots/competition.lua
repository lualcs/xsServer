...
---@class slotsCompetition:gameCompetition @slots 桌子
...
---获取唯一配置
---@return slots_icon[]
function competition:getSoleIcon()
...
function competition:getLayoutInfo()
    local info = self:getGameInfo()
    return info.layout
end
...
---获取所有图标权重配置
---@return slots_wight[][]
function competition:getDoneIconWeights()
    local cfg = self._cfg
...
function competition:getFreeIconWeights()
    local cfg = self._cfg
    local idx = cfg.free_weight_index
...
---获取必杀图标权重配置
---@return slots_wight_info[]
function competition:getKillIconWeights()
    local cfg = self._cfg
...
---获取使用权重
---@return slots_wight_info[]
function competition:getRunIconWeithts()
    return self._wgts
end

---设置使用权重
---@param wgts slots_wight_info[]
function competition:setRunIconWeithts(wgts)
    self._wgts = wgts
end


---获取当前图标权重配置
function competition:getCurrIconWeights()
    --返还率 = (产出+杀分)/总下注
    local ratio = 90
    local cfg = self._cfg
...
function competition:onRotateNormal(player,msg)
    --正常摇奖
    local cfg = self._cfg
...
---@param player    slotsPlayer              @玩家
---@param msg       slots_rotateNormal_msg   @摇奖
function competition:onRotateFree(player,msg)
    if player:getFreeCount() <= 0 then
        return false,"没有免费次数"
    end
...
---@param player        slotsPlayer              @玩家
---@param msg           msgBody              @消息
function competition:onRotateRoller(player,msg)
    --设置权重
    local wgts = self:getCurrIconWeights()
    self:setRunIconWeithts(wgts)
    --扣除成本
