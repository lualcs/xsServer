...
local bullEnum = require("bullHelper.bullEnum")
---@class bullType:pokerType
local bullType = class(pokerType)

---构造函数
function bullType:ctor()
end

---获取牌型
---@param hands pkCard[] @手牌 
---@return senum
function bullType:getCardType(hands)
    ---@type bullAlgor
...
---@return boolean 
function bullType:asBullFiveBomb(hands,analy)
    local lzs = analy.lzs
...
---@param analy bullMethod @分析数据
---@return boolean 
function bullType:bullBomb(hands,analy)
    local lzs = analy.lzs
    if not table.empty(analy.fourles) then
        return true
...
---@param analy bullMethod @分析数据
---@return boolean 
function bullType:asBullFiveSmall(hands,analy)
    ---@type bullHelper
    local hp = self._hlp
...
---@param analy bullMethod @分析数据
---@return boolean 
function bullType:asA2345(hands,analy)
...
---@param analy bullMethod @分析数据
---@return boolean 
function bullType:asBullStraight(hands,analy)
    ---癞子列表
    local lzs = analy.lzs
    ---手牌列表
    local sps = analy.sps
...
---@param analy bullMethod @分析数据
---@return boolean 
function bullType:asBullFlush(hands,analy)
    local hlp = self._hlp
    local sps = analy.sps
    for i,card in ipairs(sps) do
        local n = i + 1
...
---@param analy bullMethod @分析数据
---@return boolean 
function bullType:asBullStraightFlush(hands,analy)
...
---@param analy bullMethod @分析数据
---@return boolean 
function bullType:asBullFiveFlower(hands,analy)
    ---@type bullHelper
    local hlp = self._hlp
    local sps = analy.sps
    for i,card in ipairs(sps) do
...
---@param analy bullMethod @分析数据
---@return boolean 
function bullType:asBullKinglet(hands,analy)
...
---@param analy bullMethod @分析数据
---@return boolean 
function bullType:asBullKing(hands,analy)
