...
---@class sgType:pokerType
local type = class(pokerType)
local senum = require("sg.enum")
...
---@return boolean
function type:isDaSanGong(hands)
    ---三公辅助
    ---@type sgHelper
    local hlp = self._hlp
...
---@param hands pkCard[] @牌
---@return boolean
function type:isXiaSanGong(hands)
    ---三公辅助
    ---@type sgHelper
    local hlp = self._hlp
    local a,b,c = hlp.getValue(hands[1]),hlp.getValue(hands[2]),hlp.getValue(hands[3])
