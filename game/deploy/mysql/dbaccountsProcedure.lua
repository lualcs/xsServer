...
return {
    ...
    --创建账号注册存储过程
    [[
      DROP PROCEDURE IF EXISTS `procedureRegisteredAccounts`;
    ...
    --创建游客登陆存储过程
    [[
      DROP PROCEDURE IF EXISTS `procedureLoginTourists`;
    ...
    --创建手机登陆存储过程
    [[
      DROP PROCEDURE IF EXISTS `procedureLoginPhone`;
      CREATE DEFINER=`root`@`%` PROCEDURE `procedureLoginPhone`(IN `@number` VARCHAR(16),IN `@password` VARCHAR(32))
      BEGIN
        SET @bindrid = 0; 
    ...
    --创建手机注册存储过程
    [[
      DROP PROCEDURE IF EXISTS `procedureRegisterPhone`;
    ...
    --创建微信登陆存储过程
    [[
      DROP PROCEDURE IF EXISTS `procedureLoginWechat`;
    ...
    --创建手机绑定存储过程
    [[
      DROP PROCEDURE IF EXISTS `procedureBindPhone`;
    ...
    --创建微信绑定存储过程
    [[
      DROP PROCEDURE IF EXISTS `procedureBindWechat`;
